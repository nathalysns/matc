...
//=================================================//
TCut totalcut = sh_cut_L + cer_cut_L;
TCut trigger = trigger_L;
TCut trigger1 = trigger1_L;
TCut trigger3 = trigger3_L;

TString arm = "L";
if(run>90000){ totalcut = sh_cut_R + cer_cut_R; arm = "R";
trigger = trigger_R;
trigger1 = trigger4_R;
trigger3 = trigger6_R;
}
...
T->Draw(Form("%s.vdc.%s.wire:%s.vdc.%s.rawtime>>hist12D",arm.Data(),vdc[i-1].Data(),arm.Data(),vdc[i-1].Data()), trigger+ totalcut,"colz");
gPad->Update();
}
...
T->Draw(Form("%s.vdc.%s.wire:%s.vdc.%s.time/1e-9>>hist12Dtime",arm.Data(),vdc[i-1].Data(),arm.Data(),vdc[i-1].Data()),trigger+ totalcut,"colz");
gPad->Update();
}
