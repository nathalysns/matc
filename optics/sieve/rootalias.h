...
const double sh_max_L   = 2;
const double cer_min_L  = 1500;
const double cer_max_L  = 50000;
...
const double tg_ph_L=0.025;
const double tg_vz_L=0.085;
TCut dp_cut_L=Form("fabs(L.tr.tg_dp)<%g",tg_dp_L);
// TCut dp_cut_L=Form("(L.tr.tg_dp>-0.035)&&(L.tr.tg_dp<%g)",tg_dp_L);
...
// Load the chained electron events from given kinematics and target
TChain* LoadChain(TString kins, TString target="Tritium", Int_t acc=0){
  TChain *t        = new TChain("T");
  TString rootfile = Form("/home/shujie/analyzer/rootfiles/%s-%s*",kins.Data(),target.Data());
  if(acc) rootfile = Form("/media/3TB/pass1/acc_%s_%s*",kins.Data(),target.Data());

  t->Add(rootfile);
  t->ls();
  if(t->IsZombie()) { 
    cout<<"Error: "<<rootfile.Data()<<" doesn't exist!"<<endl;
    return 0;
  }

  // temp fix (no EKLxe class for dummy target in EP data)
  if(!t->GetListOfBranches()->FindObject("EKLxe.x_bj")){ 
    cout<<"make alias"<<endl; 
    t->SetAlias("EKLxe.x_bj" ,"EKLx.x_bj" );
    t->SetAlias("EKLxe.nu"   ,"EKLx.nu"   );
    t->SetAlias("EKLxe.W2"   ,"EKLx.W2"   );
    t->SetAlias("EKLxe.Q2"   ,"EKLx.Q2"   );
    t->SetAlias("EKLxe.angle","EKLx.angle");
  }
  return t;
}
...
TChain* LoadOnline(Int_t run, const char* tree = "T")
{
    ...
    cout << "Can not find online replay file for run " << run << endl;
    delete tt;
    tt=0;
    return tt;
}
...
Int_t GetTimeStamp(TTree* tt, Int_t decay_days=0)
{
    ...
    if (decay_days==1){
        vector<Int_t> filldate={1508774400, 1535068800};// the first tritium cell filled on 2017.10.23, 2nd on 2018-8-24. please add newer date in ascending order
        Int_t days = 0;
